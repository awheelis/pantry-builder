...
router.get('/', async (req, res, next) => {
  try { res.json(await Staged.listStaged(req.user!.id)); } catch (e) { next(e); }
});
...
router.post('/', async (req, res, next) => {
  try {
    ...
    const staged = await Staged.stageRecipe(Number(recipe_id), req.user!.id);
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const updated = await Staged.updateScaleFactor(Number(req.params.id), Number(scale_factor), req.user!.id);
    await rebuildGroceryList(req.user!.id);
    res.json(updated);
  } catch (e) { next(e); }
});
...
router.delete('/:id', async (req, res, next) => {
  try {
    await Staged.unstageRecipe(Number(req.params.id), req.user!.id);
    await rebuildGroceryList(req.user!.id);
    res.status(204).send();
  } catch (e) { next(e); }
});
...
router.delete('/', async (req, res, next) => {
  try {
    await Staged.clearStaged(req.user!.id);
    await rebuildGroceryList(req.user!.id);
    res.status(204).send();
  } catch (e) { next(e); }
});
...
router.get('/:id/breakdown', async (req, res, next) => {
  try {
    const result = await Staged.getBreakdown(Number(req.params.id), req.user!.id);
